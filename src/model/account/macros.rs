...
macro_rules! account_query_by_key_value {
    ($keyname:expr, $keyvalue:expr) => {
        sqlx::query_as!(
            ...
            awaiting_claim, user_name, user_bio, pronouns, avatar_url, next_username_update, is_admin 
...
macro_rules! account_query_by_key {
    ($key:expr, $database:expr) => {
        match $key {
            ...
            AccountQueryKey::user_name(user_name) =>             
            {
                let (key, user_name) = ($key.as_ref(), &user_name);
                account_query_by_key_value!(key, user_name)
                    .fetch_one($database)
                    .await
                    .map_err(|e| Error::FailedToFetchAccount(e))
            },
            AccountQueryKey::user_name_incomplete(user_name_incomplete) =>             
            {
                let (key, user_name_incomplete) = ($key.as_ref(), &user_name_incomplete);
                account_query_by_key_value_incomplete!(key, user_name_incomplete)
                    .fetch_one($database)
                    .await
                    .map_err(|e| Error::FailedToFetchAccount(e))
            },
